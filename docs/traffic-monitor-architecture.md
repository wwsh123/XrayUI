...
Реализованы типизированные контракты, границы транспортного/парсерного/UI слоёв и потокобезопасный TrafficMonitorStore. Добавлена страница TrafficMonitorControl, график TrafficChartControl и TrafficMonitorViewModel, работающая через ITrafficMonitorReader. XrayStatsClient получает счётчики через постоянный локальный gRPC-канал StatsService; при сбое мониторинг показывает gap и не вмешивается в работающий прокси.
...
XrayTrafficCollector, gRPC-клиент, Store и UI подключены. Access-события по-прежнему разбираются отдельно из LogReceived. В MainViewModel монитор получает один Store на жизненный цикл приложения и начинает новую сессию при запуске ядра.
...
1. Проверить Native AOT/trim и Windows release-публикацию с сгенерированным gRPC-кодом.
2. Реализовать XrayAccessEventParser с реальными вариантами IPv6, >>/->, accepted/rejected, маскированием и нестандартными тегами.
3. Реализовать collector/facade: очереди, таймер, TimeProvider/монотонные метки, cancellation, rebind после рестарта, отписка при закрытии. Ошибки наблюдения не должны останавливать прокси.
